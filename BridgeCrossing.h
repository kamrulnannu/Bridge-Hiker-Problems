...
class HikerInfo
{
	private: 
		string HikerName; 
		int HikerSpeed;
		int  TimeToCrossABridgeOnce;

	public:
	HikerInfo(const string name = "", const int speed = 0, const int BridgeLen = 0) :
		  HikerName(name),
		  HikerSpeed(speed), 
	...
	int GetTimeToCrossBridgeOnce() const
	{
...
class BridgeInfo
{
	...
	public:
		BridgeInfo(const int id = 0, const int len = 0) : 
		...
		void SetBridgeLength(const int len)
		{
			BridgeLength = len;
		}
...
class BridgeAndHikersInfo
{
	...
	void SetMinTimeToCross(int time)
	{
		MinTimeToCross = time;
	}
	...
	int GetTotalTimeToCrossThisBridge() const
	{
...
class BridgeAndHikersSolution
{
	...
	int ComputeTotalTimeOfCrossingForAllBridges() const
	{
		int time = 0;
