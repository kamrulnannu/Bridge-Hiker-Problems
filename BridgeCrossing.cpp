...
BridgeAndHikersSolution::BridgeAndHikersSolution(int MarkArraySize)
{
	...
	HikerMoveDirection = new float *[MarkArraySize];
	for (auto i = 0; (i < MarkArraySize); ++i)
	{
		HikerMoveDirection[i] = new float [2];
		memset(HikerMoveDirection[i], -1, sizeof(float) *  2);
	}
}
                                                              ...
                                                              const vector<HikerInfo> &hikers)
{
	...
	float MinTime = ComputeHikersTimeToCrossBridge(HikersCrossInfo, bhinfo, CROSS_BRIDGE, NumHiker);
...
float BridgeAndHikersSolution::ComputeHikersTimeToCrossBridge(int HikersCrossingInfo,
							    ...
							    int NumHiker)
{
	...
	float & MinTime = HikerMoveDirection[HikersCrossingInfo][moveDirection];
		...
		else
		{
			...
			for (int i = 0; (i < NumHiker); ++i)
			{
				...
				for (int j = i + 1; (j < NumHiker); ++j)
				{
					...
					{
						float SlowestTime = std::max(Hikers.at(i).GetTimeToCrossBridgeOnce(),
